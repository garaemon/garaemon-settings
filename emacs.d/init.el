...
If the file is new, it will be populated with a default template."
  ...
  (defun org-agenda-quick ()
    "Call org-agenda with large GC threshold and some modes disabled. It speeds up org-agenda."
    (interactive)
    (let ((started-at (float-time))
      ...
      (let* ((ended-at (float-time))
        ...
        (message "org-agenda took %s sec" delta-duration)
        )))
