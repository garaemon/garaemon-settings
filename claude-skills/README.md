...
- [voice-diary](.claude/skills/voice-diary/SKILL.md) — Turn a dictated message (Claude Code voice input, or the Claude app on a phone) into a diary entry in the user's org-roam daily note: clean the speech-to-text transcript into readable Japanese without changing what was said, append it as a timestamped `:voice:` entry via a small host-side script (creating the note with an org-roam header when the day has none), then commit and push the org repo after the user confirms, and open the note in Emacs when a server is reachable. The free-form counterpart to `daily-wrapup`.
